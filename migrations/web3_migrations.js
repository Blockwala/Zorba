...
var web3Socket = new Web3(new Web3.providers.WebsocketProvider(config.node_socket_address));
...
* Aim: To get all Transfer events from start to stop block
*
* @Params:
...
helper.getAllTransferEvents = function(contractAddress, start, stop) {

    var options = {}
    if(start) {
        options['fromBlock'] = Number(start);
    } 

    if(stop) {
        options['toBlock'] = Number(stop);
    }
